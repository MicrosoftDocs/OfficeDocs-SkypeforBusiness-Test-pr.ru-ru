...
ms:mtpsurl: https://technet.microsoft.com/ru-ru/library/Gg398547(v=OCS.15)
ms:contentKeyID: 49310162
ms.date: 05/19/2016
...
Assigns a dial plan to one or more users or groups. Данный командлет впервые появился в Lync Server 2010.
...
In Example 2, the RedmondDialPlan dial plan is assigned to all the users who have offices in the city of Redmond. To do this, the **Get-CsUser** cmdlet is invoked in order to retrieve a collection of all the users who have an office in the city of Redmond; this is done by using the LdapFilter parameter and the LDAP query l=Redmond. (In the LDAP query language used by Доменные службы Active Directory, the l indicates a user's locality, or city.) This collection is then piped to the **Grant-CsDialPlan** cmdlet, which assigns the Redmond dial plan to each user in the collection.
