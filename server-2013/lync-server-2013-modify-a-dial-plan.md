...
4.  На странице **Dial Plan** (Абонентская группа) дважды щелкните имя абонентской группы.
    ...
    <tbody>
    <tr class="odd">
    <td>Область абонентской группы и ее имя задаются при создании абонентской группы. Они не могут быть изменены.</td>
    </tr>
    </tbody>
    </table>
...
5.  В поле **Simple name** (Простое имя) (содержит имя, отображаемое в поле **Name** (Имя)) окна **Edit Dial Plan** (Изменение абонентской группы) укажите описательное имя, которое указывает на сайт, службу или пользователя, к которым применяется абонентская группа (необязательно).
    ...
    <tbody>
    <tr class="odd">
    <td><strong>Простое имя</strong> должно быть уникальным в пределах развертывания Lync Server 2013. Длина имени не должна превышать 256 символов Юникода. Имя может содержать буквы, цифры, дефис (-), точку (.), знак &quot;плюс&quot; (+) и символ подчеркивания (_).<br />
    В поле <strong>Simple name</strong> (Простое имя) нельзя использовать пробелы.</td>
    </tr>
    </tbody>
    </table>
