...
ms:mtpsurl: https://technet.microsoft.com/ru-ru/library/Gg398561(v=OCS.15)
ms:contentKeyID: 49310178
ms.date: 05/19/2016
...
The **Start-CsWindowsService** cmdlet enables you to start a Lync Server service. Данный командлет впервые появился в Lync Server 2010.
...
Example 2 starts the "Группа ответа" service on the local computer. To do this, the command uses the Name parameter followed by the service name: RTCRGS.

    Start-CsWindowsService -Name "RTCRGS"

## EXAMPLE 3

The command shown in Example 3 also starts the "Группа ответа" service; in this case, however, the service is started on the remote computer atl-cs-001.litwareinc.com. To start a service on a remote computer, include the ComputerName parameter followed by the FQDN of the remote computer.
...
Many Lync Server components run as standard Windows services; for example, the помощника по конференц-связи is actually a service named RTCCAA. If one of your Lync Server services is currently stopped, you can restart that service by using the **Start-CsWindowsService** cmdlet.
...
Like other Windows services, some Lync Server services have a dependency on another service; for example, the Lync Server Conferencing Attendant service cannot run unless the приложения is already running. If you try to start a service that depends on another service, the **Start-CsWindowsService** cmdlet will start both of those services. That means that, if you try to start the Conferencing Attendant service, the cmdlet will first start the приложения and then start the Conferencing Attendant service. However, the **Start-CsWindowsService** cmdlet will not automatically start any dependent services of a service: if you start the приложения, the command will not automatically start the Conferencing Attendant service as well.
