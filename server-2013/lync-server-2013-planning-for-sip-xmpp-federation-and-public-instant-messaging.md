...
Включение федерации между Microsoft Lync Server 2013, Lync Server 2010 и Office Communications Server использует сервер и обратные прокси-серверы для применения правил и разрешенных доменов партнеров, которые задаются администратором. С точки зрения планирования федерация с другими Lync ServerOffice Communications Server требует выполнения перечисленных ниже действий:
  ...
  - Определение требований для обнаружения федеративного домена.
      ...
      -   
        ...
        <tbody>
        <tr class="odd">
        <td>Ручная настройка сервер федеративного партнера может привести к сбою, если партнер изменит IP-адрес своего сервера сервер.</td>
        </tr>
        </tbody>
        </table>
        ...
        <tbody>
        <tr class="odd">
        <td>Для <strong>Новых федеративных доменов SIP</strong> необходимо предоставить <strong>Доменное имя (или полное доменное имя)</strong> для Microsoft Lync Online, Microsoft Office 365. Для Microsoft Lync Server 2013, Lync Server 2010 и Office Communications Server необходимо также предоставить <strong>Пограничную службу доступа (полное доменное имя)</strong></td>
        </tr>
        </tbody>
        </table>
      ...
      -   
        ...
        <tbody>
        <tr class="odd">
        <td>Если вы поддерживаете клиентов Microsoft Lync Mobile либо на Windows Phone либо на Apple iPhone, iPad или другом устройстве Apple и используете службу служба push-уведомлений или службу служба push-уведомлений, то должны планировать записи SRV _sipfederationtls._tcp. <em>&lt;домен_SIP&gt;</em> для каждого домена SIP, в котором имеются клиенты Lync Mobile. Android и Nokia Symbian Lync Mobile не используют push-уведомления, и к ним это требование не относится.</td>
        </tr>
        </tbody>
        </table>
...
<tbody>
<tr class="odd">
<td>Федерация XMPP не поддерживается для пользователей, которые используют устройства для обеспечения связи в филиалах. Данное применимо к просмотру информации о присутствии и к обмену мгновенными сообщениями.</td>
</tr>
</tbody>
</table>
