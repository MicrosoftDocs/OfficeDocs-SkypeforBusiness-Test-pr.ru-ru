---
title: 'Lync Server 2013: сводка по портам — единая консолидированная пограничная топология с закрытыми IP-адресами и трансляцией сетевых адресов'
TOCTitle: Сводка по портам — единая консолидированная пограничная топология с закрытыми IP-адресами и трансляцией сетевых адресов
ms:assetid: 3c1a389f-5f42-4719-a05b-e0b84aa3eb9e
ms:mtpsurl: https://technet.microsoft.com/ru-ru/library/Gg425891(v=OCS.15)
ms:contentKeyID: 49309508
ms.date: 05/19/2016
...
Описанная в данном сценарии система Lync Server 2013, являющаяся функциональной возможностью пограничного сервера сервер, очень похожа на Lync Server 2010. Наиболее заметным нововведением является вход с помощью порта **5269 через TCP** для расширения возможностей протокола обмена сообщениями и сведений о присутствии (XMPP). Система Lync Server 2013 дополнительно развертывает XMPP-прокси на пограничном сервере сервер или в пограничном пуле пул, а также сервер шлюза XMPP на сервере переднего плана или пуле переднего плана.

Помимо IPv4 сервер теперь поддерживает IPv6. Для ясности в этих сценариях используется IPv4.

**Периметр сети для отдельного консолидированного пограничного сервера с частными IP-адресами, использующий NAT**
...
### Сводка по брандмауэру для единой и консолидированной среды с частными IP-адресами при использовании NAT – внешний интерфейс
...
<td><p>Пограничная служба аудио- и видеоконференций пограничного сервера</p></td>
<td><p>Любая</p></td>
<td><p>Исходящий порт 3478 используется для определения версии пограничного сервера, с которым взаимодействует Lync Server, а также для передачи мультимедийного трафика с пограничного сервера на пограничный сервер. Требуется для федерации с Lync Server 2010, Windows Live Messenger и Office Communications Server 2007 R2, а также при развертывании в компании нескольких пограничных пулов.</p></td>
</tr>
<tr class="even">
<td><p>A/V/STUN,MSTURN/UDP/3478</p></td>
<td><p>Любая</p></td>
...
### Сводка по брандмауэру для единой и консолидированной среды с частными IP-адресами при использовании NAT – внутренний интерфейс
